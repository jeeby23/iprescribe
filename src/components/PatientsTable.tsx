...
import { 
  ...
  TableHead, TableRow, Typography, Box, TextField, InputAdornment, Chip 
} from '@mui/material';
  ...
  return (
    <Paper elevation={0} sx={{ p: 3, borderRadius: '16px', border: '1px solid #F1F5F9', mt: 4 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h6" sx={{ fontWeight: 700, fontSize: '18px' }}>Recent Patients Sign Up</Typography>
      ...
      <TableContainer>
        <Table>
          <TableHead sx={{ bgcolor: '#F8FAFC' }}>
            <TableRow>
              {['#', 'Sign Up Date', 'Patient Name', 'Email Address', 'Phone Number', 'Last Seen', 'Location', 'Device', 'Status'].map((head) => (
                <TableCell key={head} sx={{ color: '#4A5568', fontWeight: 600, fontSize: '13px', borderBottom: 'none' }}>
                  {head}
                </TableCell>
              ))}
            </TableRow>
            ...
            {displayData.length > 0 ? (
              displayData.map((row, index) => (
                <TableRow key={row.id} sx={{ '&:hover': { bgcolor: '#F1F5F9' } }}>
                  ...
                  <TableCell sx={{ fontSize: '13px' }}>{row.email || row.user?.email || "N/A"}</TableCell>

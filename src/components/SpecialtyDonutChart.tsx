        ...
        <ResponsiveContainer width="50%" height="100%">
          <PieChart>
            ...
            <text
              ...
              style={{ fontSize: '24px', fontWeight: 800, fill: '#1A202C', fontFamily: 'Onest' }}
            >
              30%
            </text>
          </PieChart>
          ...
          <Grid container spacing={3}>
            {data.map((item) => (
              <Grid size={{ xs: 6 }} key={item.name}>
                <Typography
                  variant="caption"
                  sx={{ color: '#A0AEC0', fontWeight: 600, fontSize: '13px' }}
                >
                  {item.name}
                </Typography>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 0.5 }}>
                  ...
                  <Typography
                    variant="h5"
                    sx={{ fontWeight: 800, color: '#1A202C', fontSize: '20px' }}
                  >
                    {item.value}
                  </Typography>
                </Box>
